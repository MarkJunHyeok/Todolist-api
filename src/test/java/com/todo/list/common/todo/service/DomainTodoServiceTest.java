...
class DomainTodoServiceTest extends BaseServiceTest {
  ...
  void setUp() {
    ...
    this.todo = this.todoRepository.save(new Todo(user, "description", SO_SO));
  }

  @Test
  void Todo_생성_성공() {
    final Todo todo = this.todoService.create(user.getId(), "description", SO_SO);
  ...
  void TodoList_조회_성공() {
    ...
    final List<Todo> todoList =
        this.todoService.getTodoList(user.getId(), IN_PROGRESS, IMPORTANT, LocalDate.now());
    final List<Todo> yesterdayTodoList =
        this.todoService.getTodoList(
            user.getId(), IN_PROGRESS, IMPORTANT, LocalDate.now().minusDays(1));
